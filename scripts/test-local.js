...
try {
  ...
  assert.equal(health.json.latestAndroidVersion, "1.0.54");
  assert.equal(health.json.latestAndroidVersionCode, 55);
  ...
  assert.equal(packageJson.scripts["android:device-qa"], "node scripts/android-device-qa.js");
  const androidGradle = await readFile(path.join(repoRoot, "pixelgom-bridge-android", "app", "build.gradle"), "utf8");
  assert.match(androidGradle, /versionCode 55/);
  assert.match(androidGradle, /versionName "1\.0\.54"/);
  ...
  assert.match(androidEventSender, /\/api\/apply/);
  ...
  assert.match(androidMainActivity, /Google로 로그인/);
  assert.match(androidMainActivity, /Apple로 로그인/);
  assert.match(androidMainActivity, /카카오계정으로 로그인/);
  ...
  assert.match(androidMainActivity, /showApplyService/);
  assert.match(androidMainActivity, /EventSender\.applyService/);
  assert.match(androidMainActivity, /서비스 신청/);
  assert.match(androidMainActivity, /결제 확인/);
  ...
  assert.match(androidMainActivity, /showConnectCodePicker/);
  assert.match(androidMainActivity, /showConnectCodeDialog/);
  assert.match(androidMainActivity, /앱에서 연결코드 찾기/);
  assert.match(androidMainActivity, /연결코드 목록/);
  assert.match(androidMainActivity, /connectionCodeInput = input\("앱 연결코드"[\s\S]*앱에서 연결코드 찾기[\s\S]*연결코드로 방 추가\/갱신/);
  assert.match(androidMainActivity, /방 관리 열기/);
  assert.match(androidMainActivity, /내 방 관리/);
  assert.match(androidMainActivity, /테스트 체크리스트/);
  assert.doesNotMatch(androidMainActivity, /openUrl\(BUYER_CONSOLE_URL\)/);
  assert.doesNotMatch(androidMainActivity, /openUrl\(BUYER_SETUP_URL\)/);
  assert.doesNotMatch(androidMainActivity, /BUYER_CONNECT_CODE_URL/);
  ...
  assert.match(androidMainActivity, /logFilterMode/);
  assert.match(androidMainActivity, /선택 로그 복사/);
  assert.match(androidMainActivity, /전체 내보내기/);
  assert.match(androidMainActivity, /전체 삭제/);
  assert.match(androidMainActivity, /현재 필터에 맞는 로그가 없습니다/);
