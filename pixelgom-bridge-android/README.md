...
6. 앱의 `연결코드로 자동 설정` 실행
7. 필요하면 대표 방 설정에서 방 이름, roomId, 링크, 입장확인 문구, 관리자, 라이선스 키를 직접 수정
8. 추가 방은 `https://pixgom.com/admin` 관리 콘솔에서 등록
9. 방별 기능 ON/OFF에서 출석, 포인트, 랭킹, 히스토리, 게임 사용 여부 설정
10. `서버 테스트 전송`으로 서버 연결 확인
11. `테스트 체크리스트`에서 카카오방 확인 순서를 점검
12. 필요한 경우 `JS 자동응답 사용`을 켜고 스크립트를 저장
13. 카카오방에서 `/브릿지`, `/상태`, `/js상태`, `/구독상태`, `/포인트`, `/출석`, `/게임`, `/주사위` 테스트
14. 방장봇 환영문구를 방별 입장확인 문구로 맞춘 뒤 첫 채팅 히스토리 보정을 테스트
