...
6. 상세 설치값은 `https://pixgom.com/buyer-guide` 구매자 전용 가이드에서 확인
7. 추가 방은 `https://pixgom.com/admin` 관리 콘솔에서 등록
8. 방별 기능 ON/OFF에서 출석, 포인트, 랭킹, 히스토리, 게임 사용 여부 설정
9. `서버 테스트 전송`으로 서버 연결 확인
10. 필요한 경우 `JS 자동응답 사용`을 켜고 스크립트를 저장
11. 카카오방에서 `/상태`, `/브릿지`, `/js상태`, `/진단` 테스트
12. 방장봇 환영문구를 방별 입장확인 문구로 맞춘 뒤 첫 채팅 히스토리 보정을 테스트
