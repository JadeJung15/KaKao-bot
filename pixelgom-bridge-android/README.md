...
| 현재 서버 권장 | `1.0.30` / versionCode `31` |
| 내부 테스트 등록 | `1.0.30` / versionCode `31` |
| 다음 준비 AAB | `1.0.31` / versionCode `32` |
...
8. Android 1.0.31 이상에서는 성공/응답 로그, 실패/재시도 필요 로그, 무시된 알림 로그를 분리해 봅니다. 카카오 그룹 요약이나 빈 알림은 명령어 실패가 아니라 무시된 알림으로 표시됩니다.
9. 전송 실패 이벤트는 앱 내부 대기 큐에 보관되며, `전송 대기 재시도`로 서버에 다시 보낼 수 있습니다. 앱 화면에서 최근 서버 timing, 최근 처리 성공, 최근 무시 알림, 대기 이벤트 수를 확인합니다.
