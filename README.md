...
`/최근이벤트`는 임시로 누구나 사용할 수 있으며, 카톡 브릿지가 전달한 이벤트 요약과 `targetUserId/profileHash` 같은 고유값 후보를 확인할 때 사용합니다. 같은 `profileHash`로 닉네임이 바뀐 기록이 있으면 회원 테이블과 최근 원본 이벤트를 함께 보고 `회원이력`에 이전 닉네임도 표시합니다. `/원본로그`는 원본 JSON을 보여주므로 관리자 전용입니다.
