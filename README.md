...
구매자 콘솔에서 승인된 방이 있으면 설치 가능한 문구형 템플릿을 `POST /api/buyer/command-templates/install`로 방별 커스텀 명령어에 담습니다. 고정 명령어와 관리자 전용 명령어는 예약어로 보호하며, 채팅방 응답에는 요금, 라이선스 키, 운영자 토큰 같은 민감 정보를 노출하지 않습니다.
