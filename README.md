...
닉네임 관련 명령어는 프로필 조회 없이 방에 저장된 이름만 기준으로 찾습니다. 저장 닉네임이 `우주 남`, `소영 여`여도 명령할 때는 `/닉이력 우주`, `/궁합 소영`, `/송금 소영 10`처럼 성별을 빼고 입력할 수 있습니다. 답장은 실제 전체 닉네임으로 표시합니다. 같은 이름의 남/여가 둘 다 있으면 전체 닉네임으로 다시 입력해야 합니다.
...
뽑기: -10P, 아이템/보너스 획득
퀴즈 정답: +20P
낚시: +0~60P, RPG EXP/아이템
채집: +1~25P, RPG EXP/아이템
탐험: +0~35P, RPG EXP/아이템/회복/피해
사냥: 성공 시 +12~60P, 실패 시 HP 피해
...
가입 전에는 게임/포인트/순위 기능을 사용할 수 없고, 명령어를 입력하면 가입 안내가 나옵니다. 방의 첫 가입자는 자동으로 관리자가 됩니다.
