...
- 31차 명령어 스토어: 400개 명령어 템플릿 카탈로그, 구매자 방 설치 API, 자체 전용 브릿지 엔진 앱 경쟁력 문구 추가
...
`GET /command-store`와 `GET /api/command-templates`는 구매자가 명령어를 쇼핑하듯 고를 수 있는 템플릿 카탈로그입니다. 현재 카탈로그는 400개 템플릿을 반환하며, 추천 세트, 기본 운영, 참여자용, 관리자용, 확률게임, 펫키우기, RPG/탐험, 상점/아이템, 이벤트/시즌, 커뮤니티, AI 운영도우미 후보로 분류합니다.
...
명령어 스토어 화면은 추천/세트/설치 가능/게임 연결/관리자용/즐겨찾기/장바구니 빠른 보기로 400개 템플릿을 좁혀 보여줍니다. 구매자는 설치 전 명령어와 응답 문구를 직접 편집할 수 있고, 세트 템플릿은 여러 명령어와 응답을 한 번에 편집한 뒤 일괄 설치합니다. 설치된 커스텀 명령어는 `POST /api/buyer/custom-commands`와 `POST /api/buyer/custom-commands/delete`로 조회/삭제합니다.
