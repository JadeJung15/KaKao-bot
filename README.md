...
`GET /command-store`와 `GET /api/command-templates`는 구매자가 명령어를 쇼핑하듯 고를 수 있는 템플릿 카탈로그입니다. 현재 카탈로그는 400개 템플릿을 반환하며, 기본 운영, 참여자용, 관리자용, 확률게임, 펫키우기, RPG/탐험, 상점/아이템, 이벤트/시즌, 커뮤니티, AI 운영도우미 후보로 분류합니다.

구매자 콘솔에서 승인된 방이 있으면 설치 가능한 문구형/게임 연결 템플릿을 `POST /api/buyer/command-templates/install`로 방별 커스텀 명령어에 담습니다. 고정 명령어와 관리자 전용 명령어는 예약어로 보호하며, 채팅방 응답에는 요금, 라이선스 키, 운영자 토큰 같은 민감 정보를 노출하지 않습니다.

명령어 스토어 화면은 추천/설치 가능/게임 연결/관리자용/즐겨찾기/장바구니 빠른 보기로 400개 템플릿을 좁혀 보여줍니다. 구매자는 설치 전 명령어와 응답 문구를 직접 편집할 수 있고, 설치된 커스텀 명령어는 `POST /api/buyer/custom-commands`와 `POST /api/buyer/custom-commands/delete`로 조회/삭제합니다.
